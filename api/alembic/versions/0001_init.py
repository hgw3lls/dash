...
def upgrade() -> None:
    ...
    if not _index_exists("opportunities", "ix_opportunities_deadline"):
        op.create_index("ix_opportunities_deadline", "opportunities", ["deadline"])
    if not _index_exists("opportunities", "ix_opportunities_type"):
        op.create_index("ix_opportunities_type", "opportunities", ["type"])
    if not _index_exists("opportunities", "ix_opportunities_status"):
        op.create_index("ix_opportunities_status", "opportunities", ["status"])
    if not _index_exists("opportunities", "ix_opportunities_priority"):
        op.create_index("ix_opportunities_priority", "opportunities", ["priority"])
    if not _index_exists("opportunities", "ix_opportunities_updated_at"):
    ...
    if not _index_exists("tags", "ix_tags_name"):
    ...
    if not _index_exists("opportunity_tags", "ix_opportunity_tags_opportunity_id"):
        op.create_index("ix_opportunity_tags_opportunity_id", "opportunity_tags", ["opportunity_id"])
    if not _index_exists("opportunity_tags", "ix_opportunity_tags_tag_id"):
        op.create_index("ix_opportunity_tags_tag_id", "opportunity_tags", ["tag_id"])

    if not _table_exists("saved_views"):
        op.create_table(
            "saved_views",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
    ...
    if not _index_exists("ingest_runs", "ix_ingest_runs_ran_at"):
